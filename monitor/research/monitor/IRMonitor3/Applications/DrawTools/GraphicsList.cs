...
namespace DrawTools
{
    ...
    public class GraphicsList
    {
        ...
        private List<float> widthList = new List<float>();
        public bool Dirty {
            get {
                if (_isDirty == false) {
        ...
        public void Draw(Graphics g, Rectangle boundary, Single xScale, Single yScale, Color color, int width)
        {
            ...
            //graphicsList.Sort();
            if (numberObjects > number) {
                colorList.Add(color);
                widthList.Add(width);
            }
            else if (numberObjects < number) {
                colorList.Remove(colorList[number - 1]);
                widthList.Remove(widthList[number - 1]);
            }
            number = numberObjects;
            for (int i = numberObjects - 1; i >= 0; i--) {
                DrawObject o = (DrawObject)graphicsList[i];
                Pen pen = new Pen(colorList[numberObjects - 1 - i], widthList[numberObjects - 1 - i]);
                o.Draw(g, boundary, xScale, yScale, pen);
                if (o.Selected)
                    o.DrawTracker(g, boundary, xScale, yScale);
            }
        }
