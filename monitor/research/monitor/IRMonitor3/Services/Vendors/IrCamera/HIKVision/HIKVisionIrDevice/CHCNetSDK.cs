...
namespace HIKVisionIrDevice
{
    ...
    public class CHCNetSDK
    {
        ...
        public struct NET_DVR_ALARMINFO_V40
       {
        ...
        public struct NET_DVR_IPPARACFG_V40
        {
            ...
            public void Init()
            {
                byAnalogChanEnable = new byte[MAX_CHANNUM_V30];
                struIPDevInfo = new NET_DVR_IPDEVINFO_V31[MAX_IP_DEVICE_V40];
                struStreamMode = new NET_DVR_STREAM_MODE[MAX_CHANNUM_V30];
                byRes2 = new byte[20];
            }
        }
    ...
    //51-"独龙",52-"鄂伦春",53-"赫哲",54-"门巴",55-"珞巴",56-"基诺"
        ...
        {
            public uint dwSize;
            public uint dwQueryCond;//查询条件 0表示无效，1表示有效
            public uint dwChannel;//默认是1（[1~32]，bit0表示通道1，依次类推bit31表示通道32）
            public NET_DVR_TIME_V30 struStartTime;//开始时间
            public NET_DVR_TIME_V30 struEndTime;//结束时间
            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = MAX_LICENSE_LEN, ArraySubType = UnmanagedType.I1)]
            public byte[] sLicense;//(设备支持模糊查询, GB2312编码)
            public uint dwPlateType;//车牌类型（支持按位表示，可以复选）
            public uint dwPlateColor;//车牌颜色（支持按位表示，可以复选）
            public uint dwVehicleColor;//车身颜色（支持按位表示，可以复选）
            public uint dwVehicleType;//车辆类型（支持按位表示，可以复选）
            public uint dwIllegalType;
            public uint dwEventType;
            public uint dwForensiceType;
            public ushort wVehicleLogoRecog;  //车辆主品牌，参考"车辆主品牌.xlsx" （仅单选）
            public byte byLaneNo;//车道号（0~255,0号车道 表示 车道号未知）
            public byte byDirection;//监测方向，1-上行，2-下行，3-双向，4-由东向西，5-由南向北,6-由西向东，7-由北向南    
            public ushort wMinSpeed;//最低速度（0~999）单位km/h
            public ushort wMaxSpeed;//最高速度（0~999）单位km/h
        ...
        {
            public uint dwSize;
            public uint dwChannel;//默认是1（[1~32]）
            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = MAX_LICENSE_LEN, ArraySubType = UnmanagedType.I1)]
            public byte[] sLicense;
            public uint dwPlateType;//车牌类型
            public uint dwPlateColor;//车牌颜色
            public uint dwVehicleColor;//车身颜色
            public uint dwVehicleType;//车辆类型
            public uint dwIllegalType;
            public uint dwEventType;
            public uint dwForensiceType;
            public ushort wVehicleLogoRecog;  //车辆主品牌，参考"车辆主品牌.xlsx" 
            public byte byLaneNo;//车道号（0~255,0号车道 表示 车道号未知）
            public byte byDirection;//监测方向，1-上行，2-下行，3-双向，4-由东向西，5-由南向北,6-由西向东，7-由北向南    
            public ushort wSpeed;//速度（0~999）单位km/h
        ...
        {
            public NET_DVR_TIME_V30 struRelativeTime; //抓拍相对时标
            public NET_DVR_TIME_V30 struAbsTime;  //抓拍绝对时标
            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 64, ArraySubType = UnmanagedType.I1)]
            public byte[] szPicName;
        ...
        //自定义热成像全屏测温数据结构体
        [StructLayoutAttribute(LayoutKind.Sequential)]
        public struct _STREAM_FRAME_INFO_S_
        {
            public uint u32MagicNo;
            public uint u32HeaderSize;
            public uint u32StreamType;
            public uint u32StreamLen;
            public union_DataInfo DataInfo;
            public union_SuppleInfo SuppleInfo;
            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 12, ArraySubType = UnmanagedType.U4)]
            public uint[] res;
            public uint u32CrcVal; //结构体校验码 对结构体前面数据进行校验
        }

        //自定义热成像补充数据联合体
        [StructLayout(LayoutKind.Explicit)]
        public struct union_SuppleInfo
        {
            [FieldOffset(0)]
            public STREAM_FS_SUPPLE_INFO_S stRTDataInfo;
            //[FieldOffset(0)]
            //[MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 12, ArraySubType = UnmanagedType.U4)]
            //public uint[] u32Res1;
        }
        ...
        [StructLayoutAttribute(LayoutKind.Sequential)]
        public struct STREAM_RT_DATA_INFO_S
        {
            public uint u32RTDataType; // 1-14bit 裸数据; 2-全屏测温结果数据; 3-YUV 数据
            public uint u32FrmNum;
            public uint u32StdStamp; //DSP 相对时间戳
        ...
        [StructLayoutAttribute(LayoutKind.Sequential)]
        public struct STREAM_FS_SUPPLE_INFO_S
        {
            public uint u32TmDataMode; /* 0 为4 字节，1 为2 字节 */
            public uint u32TmScale; /* 测温缩放比例 */
            public uint u32TmOffset; /* 测温偏移量，当前固定为0 */
            public uint res;
        }
