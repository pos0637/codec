...
namespace Gige
{
    ...
    char* Utils::GetLocalAddr(char* ip)
    {
        if (!ip)
            return NULL;
        ...
        for (int i = 0; hostinfo->h_addr_list[i] != NULL; ++i) {
            char* temp = inet_ntoa(*((in_addr*)hostinfo->h_addr_list[i]));
            if (temp != NULL) {
    ...
    bool Utils::GetAvaliablePort(int32_t &port)
    {
        bool result = true;
    ...
    // 成功返回1，出错返回0
    // 转换成功，结果保存在结构体指针ap中
    bool Utils::GetHostByte(const char *ip, unsigned long* out)
    {
        int32_t dots = 0;
    ...
    bool Utils::GetNetworkByte(const char *ip, unsigned long* out)
    {
